...
This is very powerful feature of knucklebone. It allows you to open up an I/O pipe that can take in any number of request URL's and will deliver each response asynchronously as they are completed.   

All that is needed to start a stream is to specify the stream option when initiating the knucklebone instance `knucklebone({stream:true})`.  (*each response is returned individually, therefore the `each()` method is not needed/available*)  
#####Simple Example
```javascript
...
The following is a list of options you can set when initializing a knucklebone:    
...
- **response**  (can be an ArrayBuffer, Blob, Document, JavaScript object, or a string)
